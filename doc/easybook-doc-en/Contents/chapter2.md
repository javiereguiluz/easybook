...
The contents defined by **easybook** depend on both the edition being published and the content type. You can access these default contents on the `Contents/` directory of the theme. If you don't want to use the default content, simply add the `content` option indicating the file that defines its contents:
