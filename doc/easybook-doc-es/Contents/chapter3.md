...
Las ediciones de tipo `pdf` y `html` disponen de las siguientes plantillas (su contenido varía en cada tipo de edición):
  ...
  * `introduction.twig`
  * `license.twig`
  * `lof.twig` (lista de imágenes).
  * `lot.twig` (lista de tablas).
  * `part.twig`
  * `preface.twig`
  * `prologue.twig`
  * `table.twig`, plantilla con la que se decoran todas las tablas del libro.
  * `title.twig`
  * `toc.twig` (índice de contenidos).
...
No es algo común y casi nunca es necesario, pero puedes crear nuevos tipos de contenidos al margen de los 21 tipos definidos por **easybook**. Imagina que entre algunos capítulos quieres incluir una página con una viñeta humorística. Llamemos `cartoon` a este nuevo tipo de contenido.
